...
function CustomerContextProvider(props) {
  ...
  const [numOfCustomers, setNumOfCustomers] = useState(
    localStorage.getItem("NumOfCustomers")
  );
  ...
  function removeCustomer(customerId) {
    return axios
      .delete(`${apiAddress}customers/${customerId}`, {
        headers
      })
      .then((res) => res)
      .catch((err) => err);
  ...
  function updateCustomer(customerId, name) {
    return axios
      .put(
        `${apiAddress}customers/${customerId}`,
        {
          name
        },
        {
